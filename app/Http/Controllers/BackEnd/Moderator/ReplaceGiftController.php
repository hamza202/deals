...
class ReplaceGiftController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (request()->has('filter')) {
            ...
            return view('back-end.moderator-panel.replace_gift', compact('rows', 'noAccepts','accepts'));
    ...
    public function update($id)
    {
        try {
            ...
            if (!$row) {
                return redirect()->route('moderator.replace_gifts')->with(['error' => 'هذا الطلب غير موجود']);
            }
            ...
            if ($points < $row -> gift ->points){
                return redirect()->route('moderator.replace_gifts')->with(['error' => 'عدد نقاط المعلن غير كافية']);
            }
            ...
            return redirect()->route('moderator.replace_gifts')->with(['success' => 'تم تأكيد الطلب بنجاح']);

        } catch (\Exception $ex) {

            return redirect()->route('moderator.replace_gifts')->with(['error' => 'هناك خطا ما يرجي المحاوله فيما بعد']);
        }
    }
    public function updateNotAccept(Request $request)
    {
        try {
            ...
            return redirect()->route('moderator.replace_gifts')->with(['success' => 'تم تأكيد الرفض بنجاح']);
