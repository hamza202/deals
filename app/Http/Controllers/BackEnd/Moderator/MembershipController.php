...
class MembershipController extends Controller
{
    public function index(){
        ...
        $check = $this -> checkRole(15);
        if($check == false){
            return view('back-end.moderator-panel.notRole');
        }

        $memberships = Membership::with('advertiserMembership')->get();
        return view('back-end.moderator-panel.memberships' , compact('memberships'));
    }

    public function store(MembershipRequest $request){

        $check = $this -> checkRole(16);
        if($check == false){
            return view('back-end.moderator-panel.notRole');
        }
...
//            }
            ...
            return redirect()->route('moderator.membership')->with(['success' => 'تم اضافة العضوية بنجاح']);
        } catch (\Exception $ex) {
            return redirect()->route('moderator.membership')->with(['error' => 'هناك خطا ما يرجي المحاوله فيما بعد']);
        }
    }

    public function update(MembershipRequest $request){

        $check = $this -> checkRole(18);
        if($check == false){
            return view('back-end.moderator-panel.notRole');
        }
        ...
        try {
            ...
            if (!$row)
                return redirect()->route('moderator.membership')->with(['error' => 'هناك خطأ حاول فى وقت اخر ']);
            ...
            return redirect()->route('moderator.membership')->with(['success' => 'تم تحديث العضوية بنجاح']);
        } catch (\Exception $ex) {
            return redirect()->route('moderator.membership')->with(['error' => 'هناك خطا ما يرجي المحاوله فيما بعد']);
        }
    }
