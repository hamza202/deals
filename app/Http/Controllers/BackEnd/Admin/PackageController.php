...
class PackageController extends Controller
{
    public function index()
    {
        ...
        return view('back-end.control-panel.subscription_package', compact('rows'));
    }
    ...
    public function store(PackageRequest $request)
    {
        ...
        try {
            ...
            if (!$row) {
                return redirect()->route('admin.packages')->with(['error' => 'هناك خطأ يرجي المحاوله فيما بعد']);
            }


            return redirect()->route('admin.packages')->with(['success' => 'تم اضافة الباقة بنجاح']);

        } catch (\Exception $ex) {

            return redirect()->route('admin.packages')->with(['error' => 'هناك خطا ما يرجي المحاوله فيما بعد']);
        }
    ...
    public function update(PackageRequest $request)
    {
        ...
        try {
            ...
            if (!$row) {
                return redirect()->route('admin.packages')->with(['error' => 'هذه الباقة غير موجوده']);
            }
    ...
    public function subscription()
    {
        ...
        return view('back-end.control-panel.subscription',
    ...
    public function storeSubscription($id)
    {
        try {
            ...
            if (!$row) {
                return redirect()->route('admin.subscription.show')->with(['error' => 'هذه الباقة غير موجوده']);
            }
            ...
            return redirect()->route('admin.subscription.show')->with(['success' => 'تم تحديث الباقة بنجاح']);

        } catch (\Exception $ex) {
            return redirect()->route('admin.subscription.show')->with(['error' => 'هناك خطا ما يرجي المحاوله فيما بعد']);
        }
    }
    ...
    public function PackageAnswer(Request $request)
    {
        try {
            ...
            if (!$data) {
                return redirect()->route('admin.subscription.show')->with(['error' => 'هناك خطأ يرجي المحاوله فيما بعد']);
            }
            Notification::send($advertiser, new \App\Notifications\SubscriptionPackage($advertiser, $answer));

            return redirect()->route('admin.subscription.show')->with(['success' => 'تم ارسال الرد ']);
